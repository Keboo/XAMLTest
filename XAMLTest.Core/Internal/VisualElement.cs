...
internal class VisualElement<T> : IVisualElement, IVisualElement<T>, IElementId
{
    ...
    public async Task<Area> GetCoordinates()
    {
        ...
        if (await Client.GetCoordinatesAsync(query) is { } reply)
        {
            ...
            return new Area(reply.Left, reply.Top, reply.Right, reply.Bottom);
        }
        ...
        request.KeyboardData.AddRange(keyboardInput.Inputs.Select(i =>
        {
            ...
            switch (i)
            {
                case KeysInput keysInput:
                    rv.Keys.AddRange(keysInput.Keys);
                    break;
    ...
    public async Task<Location> SendInput(MouseInput mouseInput)
    {
        ...
        if (await Client.SendInputAsync(request) is { } reply)
        {
            ...
            return new Location(reply.CursorX, reply.CursorY);
        }
    ...
    public IVisualElement<TElement> As<TElement>()
    ...
    public async Task Highlight(HighlightConfig highlightConfig)
    {
        ...
        {
            ElementId = Id,
            BorderColor = Serializer.Serialize(typeof(Color), highlightConfig.BorderColor),
            BorderThickness = highlightConfig.BorderThickness,
            OverlayColor = Serializer.Serialize(typeof(Color), highlightConfig.OverlayColor),
